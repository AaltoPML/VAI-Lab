...
_PLUGIN_READABLE_NAMES = {"Reinvent": "default",
                          "reinvent": "alias"}   # type:ignore
_PLUGIN_MODULE_OPTIONS = {"Type": "scaler"}                         # type:ignore
_PLUGIN_REQUIRED_SETTINGS = {}                                      # type:ignore
_PLUGIN_OPTIONAL_SETTINGS = {}                                      # type:ignore
_PLUGIN_REQUIRED_DATA = {}                                          # type:ignore
_PLUGIN_OPTIONAL_DATA = { "acquisition", "seed"}                # type:ignore


class Reinvent(DataProcessingT):
    """
    Given a scoring function, generates a set of molecules
    """
    
    # --------- change these path variables as required
    #reinvent_dir = os.path.expanduser("/path/to/Reinvent")
    reinvent_dir = os.path.expanduser("/u/87/asadim1/unix/Desktop/Research/Codes/HITL-drugdesign/reinvent-hitl/Reinvent")
    #reinvent_env = os.path.expanduser("/path/to/conda_environment") 
    reinvent_env = os.path.expanduser("/u/87/asadim1/unix/anaconda3/envs/reinvent.v3.2/")
    ...
    def __init__(self, config = {}, data_in = [None], ini = False):
        ...
        if not ini:
            ...
            try:    
                self.model = ReinventModel(**self._config["options"])
            ...
            self.model = ReinventModel
            
        self.fit_plugin = self.model.do_run
...
class ReinventModel:
	...
	# Quick options
        ...
        K = 4 # number of REINVENT runs (K=R+1 in the paper): usage: K=2 for one HITL round (T*n+n0 queries); K=3 for two HITL rounds (2*(T*n+n0) queries)
        ########################################
	...
	#reinvent_env = os.path.expanduser("/path/to/conda_environment") 
        reinvent_env = os.path.expanduser("/u/87/asadim1/unix/anaconda3/envs/reinvent.v3.2/")
        output_dir = os.path.expanduser("/u/87/asadim1/unix/Desktop/Research/Codes/HITL-drugdesign/reinvent-hitl/results/{}_seed{}".format(jobid,seed))
		...
		# run REINVENT
            ...
            with open(os.path.join(output_dir, "results/scaffold_memory.csv"), 'r') as filee:
	    ...
	    # Run REINVENT again
            ...
            with open(os.path.join(output_dir_iter, "results/scaffold_memory.csv"), 'r') as filee:
                data_it1 = pd.read_csv(filee)
	...
	# Plot the final result
        ...
        print("Printing final resutl here:"+output_dir)
        plt.close()
